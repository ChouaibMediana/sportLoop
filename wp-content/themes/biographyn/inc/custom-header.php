 ...
 *
 * @uses biographyn_header_style()
 */
function biographyn_custom_header_setup() {
	add_theme_support( 'custom-header', 
		apply_filters( 'biographyn_custom_header_args', 
			array(
				...
				'height'                 => 528,
				'flex-height'            => true,
				'wp-head-callback'       => 'biographyn_header_style',
			)
		)
	);
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'biographyn' ),
		),
	) );
}
add_action( 'after_setup_theme', 'biographyn_custom_header_setup' );

if ( ! function_exists( 'biographyn_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see biographyn_custom_header_setup().
	 */
	function biographyn_header_style() {
		...
		wp_add_inline_style( 'Biographyn-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'biographyn_header_style', 10 );